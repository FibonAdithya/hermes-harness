...
Telegram message
  ...
  -> shell/git tool calls against the named repo
  -> local execution (train/benchmark/etc.)
...
- Docker/sandbox isolation of executed code — explicitly declined; this is
  a dedicated single-purpose machine, so code runs directly on the host.
